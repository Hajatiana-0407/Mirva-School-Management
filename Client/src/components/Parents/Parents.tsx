...
import { Plus, Search, Filter, Edit, Archive, Eye, User, Users, UserCheck, Phone, FolderOpen, Home, Mail, PenBox, Check, X } from 'lucide-react';
...
const Parents = () => {
  ...
  const [editingParent, setEditingParent] = useState<ParentStudentType | null>(null);
    ...
    { key: 'tuteur_email', label: 'Email' },
    {
      key: 'type', label: 'Relation', render: (value: string, item: ParentType) => (
        <span className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium ${getParentRelationColor(value)}`}>
          ...
          <span>{value === 'parent' ? value.toLocaleUpperCase() : item?.tuteur_lien != '' ? item?.tuteur_lien?.toLocaleUpperCase() : value?.toLocaleUpperCase()}</span>
        </span>
      )
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={editingParent ? 'Modification' : 'Ajout Parent / Tuteur'}
        size='lg'
      >
