...
const DocumentImage = ({ url = '', label = 'photo' }: { url: string, label?: string }) => {
    ...
    return (
        <div className={clsx({
            'h-[5rem]': !isShow
        }, "w-full p-3 md:p-6 border rounded bg-blue-100  relative overflow-hidden transition-all duration-700")} >
            <a href={url} target="_blank" className="absolute top-2 left-4 italic px-3 md:px-6 font-bold rounded-full bg-blue-600 text-white hover:underline cursor-pointer"> {label} </a>
            <button
