...
const Header = () => {
  ...
  return (
    <header className="bg-white shadow-sm border-b border-gray-100 px-3 md:px-6 py-2 sticky top-0 z-40">
      <div className="flex items-center justify-between">
        {/* Section Recherche */}
        <div className="flex items-center flex-1 max-w-2xl">
          <div className="relative w-full">
            <Search className="w-4 h-4 absolute left-3 md:left-4 top-1/2 transform -translate-y-1/2 text-gray-400" />
            <input
              type="text"
              placeholder="Rechercher..."
              className="w-full pl-10 md:pl-12 pr-4 py-2 md:py-2.5 border border-gray-200 rounded-lg md:rounded-xl focus:outline-none focus:border-primary-500 bg-white transition-colors text-sm md:text-base"
            />
          </div>
        </div>
        ...
        <div className="flex items-center sm:ml-6">
          {/* Bouton Messages */}
          <button
            className="relative p-3 text-gray-500 hover:text-primary-600 hover:bg-primary-50 rounded-xl transition-all duration-200 group border border-transparent hover:border-primary-100"
            title="Messages"
          >
          ...
          <div className="relative" ref={profileRef}>
            <button
              ...
              className="flex items-center p-2 rounded-xl hover:bg-gray-50 transition-all duration-200 group border border-transparent hover:border-gray-200"
            >
              <div className="hidden md:block text-right mr-2">
                ...
                <p className="text-xs text-gray-500 flex items-center justify-end gap-1.5 mt-0.5">
              ...
              <div className="flex items-center">
                {auth.info?.photo ? (
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center overflow-hidden border-2 border-gray-100 group-hover:border-primary-200 transition-all duration-200 shadow-sm">
                    <img
                ...
                <ChevronDown className={`w-4 h-4 ml-2 text-gray-400 transition-transform duration-200 ${isProfileOpen ? 'rotate-180' : ''
                  }`} />
              </div>
            </button>
            ...
            {/* Dropdown Profil */}
            {isProfileOpen && (
              <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
                {/* En-tête du profil */}
                ...
                <div className="py-2">
                  <Link
                    ...
                    className="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-primary-50 hover:text-primary-600 transition-colors group"
                    onClick={() => setIsProfileOpen(false)}
                  >
                    <User className="w-4 h-4 mr-3 text-gray-400 group-hover:text-primary-500 transition-colors" />
                  ...
                  <Link
                    ...
                    onClick={() => setIsProfileOpen(false)}
                  >
                    <Settings className="w-4 h-4 mr-3 text-gray-400 group-hover:text-primary-500 transition-colors" />
