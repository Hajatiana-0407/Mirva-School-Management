...
import { deleteExercice, publish } from './redux/ExerciceAsyncThunk';
...
import { getExerciceBySlug, getExerciceSingleState } from './redux/ExerciceSlice';
import { getAppState } from '../../Redux/AppSlice';
import { useHashPermission } from '../../Hooks/useHashPermission';

const ExerciceSingle = () => {
    const { slug } = useParams();
    const dispatch: AppDispatch = useDispatch();
    const navigate = useNavigate();
    const { data: exercice, action } = useSelector(getExerciceSingleState);
    ...
    const handlePublish = async () => {
        if (exercice?.id_exercice) {
            await dispatch(publish(exercice.id_exercice));
        }
    };
    ...
    useEffect(() => {
        if (slug !== '') {
            dispatch(getExerciceBySlug(slug as string))
        }
        return () => { }
    }, [dispatch, slug]);
    useEffect(() => {
        ...
        <div className="space-y-4 md:space-y-6">
            ...
            <div className="p-3 md:p-6 space-y-4 md:space-y-6 bg-gradient-to-b from-white to-gray-50 shadow-md border border-gray-200 rounded-2xl transition hover:shadow-lg duration-200">
                ...
                <div className="flex flex-col md:flex-row items-center sm:justify-between gap-4 border-b-2 pb-4">
                    <div className="">
                        <Profile
                            fullName={exercice.nom ? `${exercice.nom} ${exercice.prenom}` : 'Administrateur'}
                            photo={exercice.photo as string}
                            copy={false}
                        />
                ...
                <section className="space-y-3">
                    ...
                    <div className="rounded-xl overflow-hidden">
                        <RenderFilePreview url={baseUrl(exercice.ficher_principale)} />
                    </div>
                </section>
                ...
                {exercice.fichier_support && (
                    <section className="space-y-3">
                        ...
                        <div className="rounded-xl overflow-hidden">
                            <RenderFilePreview url={baseUrl(exercice.fichier_support)} />
                        </div>
                    </section>
                )}
            </div>
            ...
            <Modal
                ...
                size="lg"
            >
                <ExerciceForm exercice={exercice} handleCloseModal={() => setShowModal(false)} />
            </Modal>
            <ConfirmDialog
                ...
                message={`Êtes-vous sûr de vouloir supprimer la leçon "${exercice.titre}" ?`}
            />
